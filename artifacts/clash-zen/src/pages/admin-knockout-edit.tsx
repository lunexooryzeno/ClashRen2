  ...
  useEffect(() => {
    ...
    authFetchAdmin(`/admin/tournaments/${tournamentId}`)
      ...
      .then((t: any) => {
        ...
        const { timeSlots, slotWindowLabel, slotEndTime, registrationCloseMinutes, enabledSlots, fixedMatchTime: savedFixedMatchTime, ...restMs } = ms;
    ...
    <div className="min-h-screen flex flex-col" style={{ background: "#0a0a0b" }}>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-5 space-y-6 pb-32" style={{ scrollbarWidth: "none" }}>
        ...
        <div className="rounded-2xl p-4 space-y-3" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)" }}>
          ...
          {Object.entries(form.matchSettings.gameplayRules).filter(([key, enabled]) => enabled && !["headshotsOnly","bodyShotsAllowed","revivalsAllowed","unlimitedAmmo","airdropsEnabled","vehiclesEnabled","horseAllowed","characterSkills","petsAllowed","glooWallAllowed","grenadesAllowed","snipersAllowed","meleeOnly"].includes(key)).map(([key]) => (
