      ...
      <div className="min-h-screen flex flex-col" style={{ background: "#0a0a0b" }}>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-5 space-y-5 pb-32" style={{ scrollbarWidth: "none" }}>
          ...
          <div className="rounded-2xl p-4 space-y-4" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)" }}>
            ...
            {/* Template: Default Timing Mode */}
            <Field label="Default Timing Mode" hint="Sets which mode is pre-selected when a match is created from this template.">
              <div className="grid grid-cols-2 overflow-hidden rounded-2xl" style={{ border: "1.5px solid rgba(255,255,255,0.10)" }}>
                <button type="button" onClick={() => setTd("fixedMatchTime", null)}
                  className="flex items-center justify-center gap-2 py-2.5 text-[11px] font-bold transition-all"
                  style={{
                    background: !td.fixedMatchTime ? "rgba(56,189,248,0.15)" : "rgba(255,255,255,0.03)",
                    borderRight: "1px solid rgba(255,255,255,0.08)",
                    color: !td.fixedMatchTime ? "#7dd3fc" : "#52525b",
                  }}>
                  <div className="w-3 h-3 rounded-full border-2 flex items-center justify-center shrink-0"
                    style={{ borderColor: !td.fixedMatchTime ? "#38bdf8" : "#3f3f46" }}>
                    {!td.fixedMatchTime && <div className="w-1.5 h-1.5 rounded-full bg-sky-400" />}
                  </div>
                  Slot Timing
                </button>
                <button type="button" onClick={() => setTd("fixedMatchTime", td.fixedMatchTime || "20:00")}
                  className="flex items-center justify-center gap-2 py-2.5 text-[11px] font-bold transition-all"
                  style={{
                    background: !!td.fixedMatchTime ? "rgba(168,85,247,0.15)" : "rgba(255,255,255,0.03)",
                    color: !!td.fixedMatchTime ? "#c084fc" : "#52525b",
                  }}>
                  <div className="w-3 h-3 rounded-full border-2 flex items-center justify-center shrink-0"
                    style={{ borderColor: !!td.fixedMatchTime ? "#a855f7" : "#3f3f46" }}>
                    {!!td.fixedMatchTime && <div className="w-1.5 h-1.5 rounded-full bg-violet-400" />}
                  </div>
                  Fixed Start Time
                </button>
              </div>
              {!!td.fixedMatchTime && (
                <div className="mt-2">
                  <input
                    type="time"
                    value={td.fixedMatchTime}
                    onChange={e => setTd("fixedMatchTime", e.target.value || "20:00")}
