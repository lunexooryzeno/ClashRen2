...
interface PaymentSessionRecord {
  id: number; userId: number; baseRupees: number; offsetPaise: number;
  status: string; expiresAt: string; createdAt: string;
  bharatpeTxnId: string | null; topupRequestId: number | null;
  phone: string | null; inGameName: string | null;
}

const SESSION_STATUS_CFG: Record<string, { label: string; color: string; bg: string; border: string }> = {
  active:    { label: "ACTIVE",    color: "text-amber-400",  bg: "rgba(245,158,11,0.1)",   border: "rgba(245,158,11,0.3)" },
  completed: { label: "PAID",      color: "text-emerald-400",bg: "rgba(16,185,129,0.1)",   border: "rgba(16,185,129,0.25)" },
  expired:   { label: "EXPIRED",   color: "text-zinc-500",   bg: "rgba(255,255,255,0.04)", border: "rgba(255,255,255,0.1)" },
  cancelled: { label: "CANCELLED", color: "text-red-400",    bg: "rgba(239,68,68,0.08)",   border: "rgba(239,68,68,0.2)" },
};

function LiveSessionsPanel() {
  const [sessions, setSessions] = useState<PaymentSessionRecord[]>([]);
  const [loading, setLoading] = useState(true);
  const [lastUpdated, setLastUpdated] = useState<Date | null>(null);

  const load = useCallback(async () => {
    try {
      const res = await saFetch("/super-admin/payment-sessions");
      if (res.ok) {
        const data = await res.json() as PaymentSessionRecord[];
  ...
  const fmtAmount = (base: number, offset: number) => {
    const total = base + offset / 100;
    return `₹${total.toFixed(offset > 0 ? 2 : 0)}`;
  };
    ...
    <div className="flex flex-col gap-2.5">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <p className="text-xs font-bold text-zinc-400 uppercase tracking-wider">Live Payment Sessions</p>
          {active.length > 0 && (
            <span className="px-1.5 py-0.5 rounded-full text-[9px] font-bold text-amber-400"
              style={{ background: "rgba(245,158,11,0.15)", border: "1px solid rgba(245,158,11,0.3)" }}>
              {active.length} active
            </span>
          )}
        </div>
        <button onClick={load} className="text-[9px] text-zinc-600 hover:text-zinc-400 transition-colors flex items-center gap-1">
          <RefreshCw className="w-3 h-3" />
          {lastUpdated ? `${fmtAgo(lastUpdated.toISOString())}` : ""}
        </button>
      </div>

      {sessions.length === 0 ? (
        <div className="rounded-2xl py-5 text-center" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)" }}>
          <p className="text-[11px] text-zinc-600">No payment sessions yet</p>
        </div>
      ) : (
        <div className="flex flex-col gap-1.5">
          {[...active, ...recent].map(s => {
            const cfg = SESSION_STATUS_CFG[s.status] ?? SESSION_STATUS_CFG.expired;
            const user = s.inGameName ?? s.phone ?? `UID ${s.userId}`;
            return (
              <div key={s.id} className="rounded-xl px-3 py-2.5 flex items-center gap-2.5"
                style={{ background: cfg.bg, border: `1px solid ${cfg.border}` }}>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <span className="text-xs font-bold text-white">{fmtAmount(s.baseRupees, s.offsetPaise)}</span>
                    <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full ${cfg.color}`}
                      style={{ background: "rgba(0,0,0,0.2)" }}>{cfg.label}</span>
                    {s.bharatpeTxnId && <span className="text-[9px] text-emerald-400 font-mono truncate max-w-[80px]">{s.bharatpeTxnId.slice(-8)}</span>}
                  </div>
                  <p className="text-[9px] text-zinc-500 mt-0.5 truncate">{user}</p>
                </div>
                <div className="text-right shrink-0">
                  {s.status === "active" ? (
                    <p className="text-[10px] font-bold text-amber-400 font-mono">{timeLeft(s.expiresAt)}</p>
                  ) : (
                    <p className="text-[9px] text-zinc-600">{fmtAgo(s.createdAt)}</p>
                  )}
                  <p className="text-[9px] text-zinc-700">#{s.id}</p>
                </div>
              </div>
            );
          })}
